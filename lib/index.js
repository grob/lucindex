...
Index.prototype.query = function(field, search, filter, limit) {
    limit = limit || 50;
    var query = function(f, index) {
        var qp = new QueryParser(index.version, f, index.analyzer);
        var q = qp.parse(search);
        return index.searcher.search(q, filter || null, limit);
    };
    var topDocs = false;
    if (Object.prototype.toString.call(field) == "[object Array]") {
        var tDocs = [];
        for each (var f in field) {
            tDocs.push(query(f, this));
        }
        topDocs = TopDocs.merge(null, limit, tDocs); 
    } else {
        topDocs = query(field, this);
    }
    return {
