 ...
 * @example
 * const {Index} = require("lucindex");
 * const {Document, Field} = org.apache.lucene.document;
...
const {DirectoryReader, IndexWriter, IndexWriterConfig, Term} = org.apache.lucene.index;
const {SearcherManager, SearcherFactory} = org.apache.lucene.search;
 ...
 * @constructor
 */
const Index = module.exports = function(directory, analyzer) {

    let indexer = null;
    let reader = null;
    let searcherManager = null;
    const lock = {};
    ...
    Object.defineProperties(this, {
        ...
        "reader": {
            "get": sync(function() {
                ...
                const newReader = DirectoryReader.openIfChanged(reader);
                if (newReader !== null) {
 ...
 * @ignore
 */
Index.initIndex = (directory, analyzer, forceCreate) => {
    analyzer || (analyzer =  analyzerFactory.createStandardAnalyzer());
    const config = new IndexWriterConfig(analyzer);
    if (forceCreate === true) {
        config.setOpenMode(IndexWriterConfig.OpenMode.CREATE);
    } else {
        config.setOpenMode(IndexWriterConfig.OpenMode.CREATE_OR_APPEND);
    }
    const writer = new IndexWriter(directory, config);
    // initially create the index (if necessary) by doing an empty commit
    writer.close();
    return new Index(directory, analyzer);
};
...
Index.createRamIndex = function(analyzer) {
    return Index.initIndex(Index.initRamDirectory(), analyzer, true);
};
 ...
 * @type Index
 */
Index.createIndex = (dir, name, analyzer, forceCreate) => {
    return Index.initIndex(Index.initDirectory(dir, name), analyzer, forceCreate);
};
