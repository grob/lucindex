...
SimpleIndex.prototype.query = function(query, limit) {
    ...
    return new Result(this.index.searcher.search(query, limit), 
            (new Date()) - start, this);
};
 ...
 * @constructor
 */
var Result = function(topdocs, querytime, si) {
    this.size = function() {
    ...
    this.get = function(idx) {
        ...
        if (docStore[idx] != undefined) {
            return docStore[idx];
        }
        var doc = this.si.index.searcher.doc(topdocs.scoreDocs[idx].doc);
        if (doc == null)
            return (docStore[idx] = null);
        var it = doc.iterator();
        var result = {};
        while (it.hasNext()) {
            var indexableField = it.next();
            var name = indexableField.name();
            var val = si.getConverter(name).toJavaScript(indexableField);
            if (!val)
                continue;
            result[name] = val;
        }
        return (docStore[idx] = result);
    };
