...
exports.createLanguageSpecificAnalyzer = function(lang, config) {
    ...
    case "de":
        if (!config) {
            return new analysisPKG.de.GermanAnalyzer(utils.resolveVersion());
        }
        var version = utils.resolveVersion(config.version);
        if (!config.stopwords && !config.stemexclusions) {
            return new analysisPKG.de.GermanAnalyzer(version);
        }
        return new analysisPKG.de.GermanAnalyzer(version, config.stopwords, config.stemexclusions);
    default:
